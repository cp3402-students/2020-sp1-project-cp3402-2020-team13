var cpmp = function ($) {
            ...
            .on('click', function () {
                var media = wp.media(
                    ...
                    function () {
                        ...
                        player = '[cpm-player skin="' + ((skin.length) ? skin.val() : 'classic-skin') + '" width="100%" playlist="true" type="' + type + '"]' + playlist + '[/cpm-player]';
    ...
    jQuery('.for-sale').bind('click', function (evt) {
        var t = $(evt.target);
        if (t.hasClass('button_for_upload_cpmp')) {
            var file_path_field = t.parent().find('.file_for_sale');
            var media = wp.media(
                {
                    title: 'Select Media File',
                    button: {
...
jQuery(
    function ($) {
        ...
        $('select#player_id').on('change', function (evt) {
            var e = $(evt.target);
            $('#avp_display_shortcode').html('[cpm-player id="' + e.val() + '"]');
        });
