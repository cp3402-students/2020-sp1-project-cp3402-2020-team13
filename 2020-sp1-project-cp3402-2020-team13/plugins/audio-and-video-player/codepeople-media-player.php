...
if (!function_exists('cpmp_settings_link')) {
    ...
    */
    ...
    {
        $settings_link = '<a href="options-general.php?page=codepeople-media-player.php">' . __('Settings', CPMP_LANG) . '</a>';
...
if (!function_exists("cpmp_admin_menu")) {
    function cpmp_admin_menu()
    {
        ...
        // Add to admin_menu
        add_options_page(__('Audio And Video Player', CPMP_LANG), __('Audio And Video Player', CPMP_LANG), 'edit_posts', basename(__FILE__), array(&$cpmp_obj, 'admin_page'));
        add_menu_page(__('Audio And Video Player', CPMP_LANG), __('Audio And Video Player', CPMP_LANG), 'edit_pages', basename(__FILE__) . '_settings_page', array(&$cpmp_obj, 'admin_page'));
    }
}
...
if (!function_exists('cpmp_init')) {
    function cpmp_init()
    {
        ...
        // I18n
        load_plugin_textdomain(CPMP_LANG, false, dirname(plugin_basename(__FILE__)) . '/languages/');
        ...
        if (!is_admin()) {
            ...
            if (is_object($cpmp_obj)) $cpmp_obj->preview();
        } else {
