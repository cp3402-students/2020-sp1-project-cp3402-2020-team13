...
class Elementor_AVP_AUDIO_Widget extends Widget_Base
{
    ...
    private function _get_shortcode()
    {
        ...
        if (empty($shortcode)) $shortcode = '[cpm-player type="audio" /]';
    ...
    protected function _register_controls()
    {
        $this->start_controls_section(
            'avp_section',
            array(
                'label' => __('Audio Player', CPMP_LANG)
            )
        );
        ...
        $this->add_control(
            'cpm_player_skin',
            array(
                'label' => __("Player's skin", CPMP_LANG),
                'type' => 'cpmskinselect',
        ...
        $this->add_control(
            ...
            array(
                'label' => __("Player's shortcode", CPMP_LANG),
        ...
        $this->add_control(
            ...
            array(
                'show_label' => false,
                'text' => __('Select files', CPMP_LANG),
                'type' => 'cpmmediabutton',
...
class Elementor_AVP_VIDEO_Widget extends Widget_Base
{
    ...
    private function _get_shortcode()
    {
        ...
        if (empty($shortcode)) $shortcode = '[cpm-player type="video" /]';
    ...
    protected function _register_controls()
    {
        $this->start_controls_section(
            'avp_section',
            array(
                'label' => __('Video Player', CPMP_LANG)
            )
        );
...
class Elementor_AVP_GALLERY_Widget extends Widget_Base
{
    ...
    private function _get_shortcode()
    {
        ...
        if (!empty($player_id)) $shortcode = '[cpm-player id="' . esc_attr($player_id) . '" /]';
        return $shortcode;
    } // End _get_shortcode
    ...
    protected function _register_controls()
    {
        ...
        $this->start_controls_section(
            'avp_section',
            array(
                'label' => __('Player From Gallery', CPMP_LANG)
            )
        );
        ...
        if (count($result)) {
            foreach ($result as $player) {
                ...
                if (empty($default_player)) $default_player = $player->id;
            }
        }
        ...
        $this->add_control(
            'cpm_player_list',
            array(
                'label' => __("Select the player", CPMP_LANG),
                'type' => Controls_Manager::SELECT,
                'default' => $default_player,
                'options' => $players,
                'description' => '<a href="options-general.php?page=codepeople-media-player.php">' . __('Create or edit players', CPMP_LANG) . '</a>'
            )
        );
...
Plugin::instance()->widgets_manager->register_widget_type(new Elementor_AVP_AUDIO_Widget);
Plugin::instance()->widgets_manager->register_widget_type(new Elementor_AVP_VIDEO_Widget);
Plugin::instance()->widgets_manager->register_widget_type(new Elementor_AVP_GALLERY_Widget);